...
class Mainframe(tk.Frame):
    ...
    def __init__(self, master, arg=None, *args, **kwargs):
        ...
        self.port_list = list(range(5200, 5210)) + \
                         list(range(9200, 9211)) + \
                         list(range(18815, 18821)) + \
                         ['5002']
        ...
        self.port= tkinter.ttk.Combobox(self, textvariable=self.server_port, values=self.port_list, width=6)
        if self.arg.port in self.port_list:
            ...
            self.port.config(values=self.port_list, width=6)
            self.port.current(0)
    ...
    def get_distance(self, ip_address):
        ...
        try:
            result = self.calculate_distance(self.ip_info[self.local_ip]['ip_info']['coords'],
                                             self.ip_info[ip_address]['ip_info']['coords'])
    ...
    def get_map(self, a, b):
        ...
        if self.arg.google_api_key is None:  # use Yandex
            if a == b:
                url = 'http://static-maps.yandex.ru/1.x/?lang=en_US&size=%s,%s&z=10&l=map&pt=%s,pm2gnl' % (
                    map_size, map_size, self.reverse_lat_long(a))
            else:
                url = 'http://static-maps.yandex.ru/1.x/?lang=en_US&size=%s,%s&l=map&pt=%s,pm2gnl~%s,pm2rdl' % (
                    map_size, map_size, self.reverse_lat_long(a), self.reverse_lat_long(b))
            self.print('getting map from Yandex: %s' % url)
        else:

            if a == b:
                url = 'https://maps.googleapis.com/maps/api/staticmap?size=%sx%s&format=gif&maptype=roadmap&zoom=12&center=%s&markers=color:green%%7Clabel:L%%7C%s%s' % (
                    map_size, map_size, a, a, self.google_api_key)
            else:
                c = ''
                if self.distance > 500:
                    c = '&path=color:0x0000ff%%7Cweight:5%%7Cgeodesic:true%%7C%s%%7C%s' % (a, b)
                url = 'https://maps.googleapis.com/maps/api/staticmap?size=%sx%s&format=gif&maptype=roadmap&markers=color:green%%7Clabel:L%%7C%s&markers=color:red%%7Clabel:S%%7C%s%s%s' % (
                    map_size, map_size, a, b, c, self.google_api_key)
    ...
    def run_iperf(self):
        ...
        if result:
            ...
            self.ip_address = self.get_real_ip(
                ip_address)  # current ip address of server (use external for private address)
    ...
    def run_iperf3(self, upload=False):
        ...
        self.meter.set(0)
        self.meter.show_max = self.max_options.index(
            self.max_mode_value.get())  # get index of item selected in max options
        ...
        while not self.done:
            try:
                ...
                if 'error' in line:
                    ...
                    try:
                        message = 'Max = %s, Avg = %s %s' % (
                            max(results_list), total if total != 0 else sum(results_list) / max(len(results_list), 1),
                            units)
                    except ValueError:
                        message = ''
                    if upload:
                    ...
                    if (int(self.threads.get()) > 1 and line.startswith('[SUM]')) or (
                            int(self.threads.get()) == 1 and 'bits/sec' in line):
                        ...
                        if 'receiver' in line:
                            ...
                            if speed > self.meter.range + (self.meter.range / 20):  # if more than 5% over-range
                                self.setrange(next((i for i in self.ranges if i >= speed),
                                                   self.ranges[-1]))  # increase range if possible
                            self.setmeter(speed)
                            self.setunits(units)
                            self.quit.update()
                            self.update_idletasks()
    ...
    def updategeography(self, *args):
        if int(self.geography.get()) == 1:
            if self.ip_address != '' and self.local_ip != '':
                ...
                if self.map is None:
                    self.map = self.get_map(self.ip_info[self.local_ip]['ip_info']['coords'],
                                            self.ip_info[self.ip_address]['ip_info']['coords'])
    ...
    def servercalback(self, *args):
        ...
        server_name = self.local_ip if self.is_ip_private(self.server.get()) else self.server.get()
        for k in list(self.ip_info.keys()):
            self.print('checking ip address: %s, server name: %s target: %s' % (
                k, self.ip_info[k].get('server', None), server_name))
...
def main():
    ...
    parser.add_argument('-I', '--iperf_exec', action="store", default='iperf3',
                        help='location and name of iperf3 executable (default=%(default)s)')
    parser.add_argument('-ip', '--ip_address', action="store", nargs='*', default=['192.168.100.119'],
                        help='default server address(\'s) can be a list (default=%(default)s)')
    parser.add_argument('-l', '--local_ip', action="store", default=None,
                        help='local public ip address, if not given, will be fetched automatically (default=%(default)s)')
    parser.add_argument('-p', '--port', action="store", default='5201', help='server port (default=%(default)s)')
    parser.add_argument('-r', '--range', action="store", type=int, default=10,
                        help='range to start with in Mbps (default=%(default)s)')
    parser.add_argument('-R', '--reset_range', action='store_false',
                        help='Reset range to Default for Upload test (default = %(default)s)', default=True)
    parser.add_argument('-m', '--max_mode', action='store', choices=max_mode_choices,
                        help='Show Peak Mode (default = %(default)s)', default=max_mode_choices[2])
    parser.add_argument('-G', '--geography', action='store_false', help='Show map data (default = %(default)s)',
                        default=True)
    parser.add_argument('-g', '--google_api_key', action="store", default=None,
                        help='your google API key (to enable google maps) (default=%(default)s)')
